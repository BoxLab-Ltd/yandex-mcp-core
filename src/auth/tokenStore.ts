...
/**
 * Default token-cache path for an app: `XDG_CONFIG_HOME`, else
 * `~/.config/<appName>/token.json`. A caller can pass an explicit path override
 * (e.g. from an env var) to {@link createFileTokenStore} instead.
 */
export function defaultTokenPath(
    ...
    return join(base, appName, 'token.json')
}
