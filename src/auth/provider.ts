...
export const NOT_AUTHENTICATED_MESSAGE =
    'Not signed in. Use the `login` tool to sign in (it opens your browser), ' +
    "or run the server's `auth` command in a terminal."
...
export class RefreshingTokenProvider implements TokenProvider {
    ...
    private async doRefresh(): Promise<string> {
        ...
        if (!refresh) {
            throw new Error(
                'No refresh token available. Sign in again with the `login` tool or the `auth` command.',
            )
        }
