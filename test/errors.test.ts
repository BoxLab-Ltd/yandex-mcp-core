...
import { errorFromResponse, YandexApiError } from '../src/http/errors.js'
...
describe('errorFromResponse', () => {
    ...
    it('marks 503/504 as retryable server errors', () => {
        expect(new YandexApiError(503, 'x').isRetryable).toBe(true)
        expect(new YandexApiError(504, 'x').isRetryable).toBe(true)
        expect(new YandexApiError(404, 'x').isRetryable).toBe(false)
    })

    it('treats client-side timeout/network errors (status 0) as retryable', () => {
        const timeout = new YandexApiError(0, 'timed out', ['timeout'])
        const network = new YandexApiError(0, 'network', ['network_error'])
        ...
        expect(new YandexApiError(400, 'bad').isTransientLocalError).toBe(false)
    })
})
