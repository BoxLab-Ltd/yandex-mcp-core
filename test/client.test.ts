...
import { YandexClient, Semaphore } from '../src/http/client.js'
import { YandexApiError } from '../src/http/errors.js'
...
describe('YandexClient request building', () => {
...
describe('YandexClient retry/backoff', () => {
    ...
    it('does not retry a non-retryable 400 and throws YandexApiError', async () => {
        let calls = 0
        const fetchImpl = mock(async () => {
            calls += 1
            return json(400, {
    ...
    it('gives up after maxRetries and rethrows', async () => {
        ...
        await expect(
            client.request('/stat/v1/data', { ids: 1 }),
        ).rejects.toBeInstanceOf(YandexApiError)
        expect(fetchImpl).toHaveBeenCalledTimes(3) // initial + 2 retries
    })
})

describe('YandexClient transport errors', () => {
    it('maps an AbortError (timeout) to a retryable YandexApiError(status 0)', async () => {
        let calls = 0
        const fetchImpl = mock(async () => {
            calls += 1
        ...
        expect((err as YandexApiError).status).toBe(0)
        expect((err as YandexApiError).errorTypes).toContain('timeout')
        expect(calls).toBe(2) // transient → initial + 1 retry, then rethrow
    })

    it('maps a network rejection to YandexApiError(status 0, network_error)', async () => {
        const fetchImpl = mock(async () => {
        ...
        expect(err).toBeInstanceOf(YandexApiError)
        expect((err as YandexApiError).status).toBe(0)
        expect((err as YandexApiError).errorTypes).toContain('network_error')
    })
})
...
describe('YandexClient concurrency', () => {
